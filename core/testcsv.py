...
def parse_csv(csv_text_content):
    ...
    While doing that calculate and return last
    maturity year to update end_year in specs
    Add functionality to read in old save file from prior version of app.
    Also returns new specs fields if read from old save file
    """
    ...
    for row in csv_reader:
        ...
        if row[0].strip().lower() == "cusip" or row[0].strip().lower() == "type":
            # these are headers that can be skipped
            continue
        elif row[0].strip().lower() == "param":
            # old version of app specs - col 2 is field and col 3 is value
            new_specs[row[1].strip().lower()] = row[2].strip().lower()
        elif row[0].strip().lower() == "add_flow":
            # add to list of additional flows in specs
            if "additional_flows" not in new_specs:
                new_specs["additional_flows"] = []
            new_specs["additional_flows"].append({'year': row[1].strip().lower(), 'amount': row[2].strip().lower()})
        elif row[0].strip().lower() == "owned_tip"
            # old version of app - add to owned tips
